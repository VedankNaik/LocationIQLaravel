...
@csrf
  <div class="flex flex-wrap -mx-2 space-y-4 md:space-y-0">
    <div class="w-full px-2 md:w-1/4">
      ...
      @if(empty($request))
      <input class="w-full h-10 px-3 text-base placeholder-gray-600 border rounded-lg focus:shadow-outline" type="text" name="query" id="query" value=""/>
      @else
      <input class="w-full h-10 px-3 text-base placeholder-gray-600 border rounded-lg focus:shadow-outline" type="text" name="query" id="query" value="{{ $request->input('query') }}"/>
      @endif
    </div>
    <div class="p-2 bg-white border-b border-gray-200">
        ...
        <a href="{{ url('/disconnect') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 ml-10 px-4 rounded">Disconnect</a>
    </div>
  </div>        
</form>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <table class="w-full">
                        ...
                        @if(isset($res->EGID))<td class="border border-4 border-blue-800 text-center">{{$res->EGID}}</td>@endif
                        @if(isset($res->Street))<td class="border border-4 border-blue-800 text-center">{{$res->Street}}</td>@endif
                        @if(isset($res->Zip))<td class="border border-4 border-blue-800 text-center">{{$res->Zip}}</td>@endif
                        @if(isset($res->Town))<td class="border border-4 border-blue-800 text-center">{{$res->Town}}</td>@endif
                            </tr> 
                        @endforeach                                  
                        </tbody>
                    </table>
