...
<body class="bg-gray-100 font-family-karla flex">
    ...
    <aside class="relative bg-sidebar h-screen w-64 hidden sm:block shadow-xl">
        ...
        <nav class="text-white text-base font-semibold pt-3">
            ...
            <a href="{{ url('/reversedashboard') }}" class="flex items-center text-white py-4 pl-6 nav-item">
                <i class="fa fa-map-marker mr-3"></i>
                Reverse Geocode
            </a>
        </nav>
    ...
    <div class="relative w-full flex flex-col h-screen overflow-y-hidden">
        <header class="w-full items-center bg-white py-2 px-6 hidden sm:flex">
            ...
            <div x-data="{ isOpen: false }" class="relative w-1/2 flex justify-end">
                <button @click="isOpen = !isOpen"
                    class="realtive z-10 w-10 h-10 rounded-full overflow-hidden border-2 border-gray-400 hover:border-gray-300 focus:border-gray-300 focus:outline-none">
                    <img src="https://img.icons8.com/fluent-systems-regular/40/000000/settings.png" />
                </button>
        ...
        <header x-data="{ isOpen: false }" class="w-full bg-sidebar py-5 px-6 sm:hidden">
            <div class="flex items-center justify-between">
                <a href="" class="text-white text-3xl font-semibold uppercase hover:text-gray-300">LocationIQ</a>
            ...
            <nav :class="isOpen ? 'flex': 'hidden'" class="flex flex-col pt-4">
                <a class="flex items-center active-nav-link text-white py-2 pl-4 nav-item">
                    <i class="fa fa-map-marker mr-3"></i>
                    Forward Geocode
                </a>
                <a href="{{ url('/reversedashboard') }}"
        ...
        <div class="w-full h-screen overflow-x-hidden border-t flex flex-col">
            <main class="w-full flex-grow p-6">
                <div class="mt-6 ml-6 mr-6">
                    <form method="POST" action="{{ url('/serverforwardvalidate') }}" class="space-y-4 text-gray-700">
                        @csrf
                ...
                <form method="POST" action="{{ url('/serverforwardvalidate') }}">
                    @csrf
                    <div class="w-full px-2">
                        @if($request)
                        <input
                                ...
                                <table class="text-left w-full border-collapse">
                                    <thead class="bg-blue-500 text-white">
                                        <tr>
                                            ...
                                            @if(isset($addresses[0] -> $column))
                                            <th
                                                class="py-2 px-6 bg-blue-lightest font-bold uppercase text-base text-grey-dark border-b border-grey-light">
                                                {{ $column }}</th>
                                            @endif
                                            @endforeach
                                            @endif
